...
VALUE_matrix <- c(5, 10, 15, 20, 25, 30, 35, 40, 45, 50, 60, 70, 80, 90, 100, 120, 140, 160)
...
df_profit <- df_profit[,-c(2:3)] # remove scenario "none"

# Robustness check: Profit per customer for different basket values
library(reshape2)
d <- melt(df_profit, id.vars="basket")
colnames(d) <- c("Basket value","Randomization procedure","Profit")

Fig_BasketProfit <- ggplot(d, aes(Basket value,Profit, col=Randomization procedure)) + 
  geom_line() +
